...
import { Sparkles, Calendar, Trash2, ChevronUp, ChevronDown, X } from 'lucide-react';
...
export const FloatingSelectionDock: React.FC<FloatingSelectionDockProps> = ({
  ...
  return (
    <aside
      ...
      className="fixed bottom-2.5 sm:bottom-6 inset-x-0 z-40 max-w-xl mx-auto px-2 sm:px-4 pointer-events-none no-print w-full"
    >
      <div className="pointer-events-auto bg-slate-900/95 backdrop-blur-md text-white border border-slate-700/90 shadow-2xl rounded-2xl overflow-hidden transition-all duration-300 ring-1 ring-white/10">
        ...
        {isExpanded && (
          <div className="p-2.5 sm:p-3 bg-slate-950/85 border-b border-slate-800 max-h-48 overflow-y-auto divide-y divide-slate-800/60">
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {selectedCourses.map((c) => (
                ...
                >
                  ...
                  <span className="truncate max-w-[100px] sm:max-w-[140px]">{c.name}</span>
                  <button
        ...
        {/* Main Floating Bar */}
        <div className="p-2 sm:p-3 flex items-center justify-between gap-1.5 sm:gap-3">
          {/* Selected Count & Expand Trigger (Right side in RTL) */}
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="flex items-center gap-1.5 sm:gap-2 text-right hover:bg-slate-800/80 px-1.5 sm:px-2 py-1 rounded-xl transition-colors shrink-0 group min-w-0"
          ...
          >
            ...
            <div className="flex items-center gap-1 font-bold text-xs sm:text-sm text-slate-100 min-w-0">
          ...
          {/* Action Buttons: Clear, View Schedule & Generate Timetable (Left side in RTL) */}
          <div className="flex items-center gap-1 sm:gap-2 shrink-0">
            {/* Clear / Reset Button: Icon-only on mobile, icon+text on desktop */}
            <button
              onClick={onClearSelection}
              className="flex items-center justify-center gap-1 text-xs font-semibold text-rose-300 hover:text-white bg-rose-950/40 hover:bg-rose-900/60 border border-rose-800/60 p-2 sm:px-2.5 sm:py-2 rounded-xl transition-all shadow-2xs active:scale-95"
              title="تصفير وحذف جميع المواد المختارة"
              aria-label="تصفير وحذف المواد المختارة"
            >
            ...
            <button
              ...
              className={`flex items-center gap-1 sm:gap-1.5 text-xs font-semibold px-2 sm:px-3 py-2 rounded-xl transition-all border shrink-0 ${
                showSchedulePreview
            ...
            >
              ...
              <span className="sm:hidden text-[11px]">
                {showSchedulePreview ? 'إخفاء' : 'الأوقات'}
              </span>
            </button>

            {/* Optimize Button: High-visibility primary action */}
            <button
              onClick={onOptimizeSchedule}
              className="flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold px-2.5 sm:px-4 py-2 rounded-xl shadow-lg shadow-blue-900/40 transition-all hover:scale-[1.02] active:scale-[0.98] border border-blue-400/30 shrink-0"
            ...
            >
              ...
              <span className="sm:hidden text-xs">توليد الجدول</span>
            </button>
          </div>
        </div>
      </div>
